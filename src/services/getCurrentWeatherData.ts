...
const API_LOCS_WEATHER_ENDPOINT = 'https://api.openweathermap.org/data/2.5/weather'

type Props = Location & { unit: UnitMeasure }

export const getCurrentWeatherData = async ({ lat, lon, unit }: Props) => {
  try {
    const unitPerReq = unit && `&units=${unit}`
    const urlRequest = `${API_LOCS_WEATHER_ENDPOINT}?lat=${lat}&lon=${lon}&appid=${MY_API_KEY}${unitPerReq}`
